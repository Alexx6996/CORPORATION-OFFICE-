...
if ($null -eq $svcObj) {
    ...
    $svcStopped = $true
} elseif ($svcObj.Status -eq "Stopped") {
...
# 3. Убить хвосты python.exe (чтобы не остался левый uvicorn на 8181)
try {
    Write-Host "[CLEANUP] Killing any remaining python.exe..."
    Get-Process python -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
    Write-Host "[CLEANUP] python.exe terminated."
} catch {
    Write-Host "[CLEANUP] cleanup error: $($_.Exception.Message)"
}
Write-Host ""
